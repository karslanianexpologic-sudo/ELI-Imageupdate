...
try {
    # ── PRIVATE NETWORK ──────────────────────────────────────────────────────
    # Enable Network Discovery (Private)
    netsh advfirewall firewall set rule group="Network Discovery" new enable=Yes profile=private | Out-Null
    Write-Host "SUCCESS: Network Discovery enabled for Private networks." -ForegroundColor Green

    # Enable File and Printer Sharing (Private)
    ...
    # ── PUBLIC NETWORK ───────────────────────────────────────────────────────
    # Enable Network Discovery (Public)
    netsh advfirewall firewall set rule group="Network Discovery" new enable=Yes profile=public | Out-Null
    Write-Host "SUCCESS: Network Discovery enabled for Public networks." -ForegroundColor Green

    # Enable File and Printer Sharing (Public)
    netsh advfirewall firewall set rule group="File and Printer Sharing" new enable=Yes profile=public | Out-Null
    Write-Host "SUCCESS: File and Printer Sharing enabled for Public networks." -ForegroundColor Green

    # ── ALL NETWORKS ─────────────────────────────────────────────────────────
    # Enable Public Folder Sharing
    netsh advfirewall firewall set rule group="File and Printer Sharing" new enable=Yes profile=any | Out-Null
    $pubFolderPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Network"
    if (-not (Test-Path $pubFolderPath)) { New-Item -Path $pubFolderPath -Force | Out-Null }
    Set-ItemProperty -Path $pubFolderPath -Name "NoPublicFolderSharing" -Value 0 -Type DWord -Force
    Write-Host "SUCCESS: Public Folder Sharing enabled for All networks." -ForegroundColor Green

    # Disable Password Protected Sharing
    $passRegPath = "HKLM:\SYSTEM\CurrentControlSet\Control\Lsa"
    Set-ItemProperty -Path $passRegPath -Name "LimitBlankPasswordUse" -Value 0 -Type DWord -Force
    Write-Host "SUCCESS: Password Protected Sharing disabled for All networks." -ForegroundColor Green

    # Also set network sharing registry values to ensure settings persist
    $sharingRegPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\HomeGroup"
    if (-not (Test-Path $sharingRegPath)) { New-Item -Path $sharingRegPath -Force | Out-Null }

    # Ensure Function Discovery services are running (required for Network Discovery)
    $fdServices = @("FDResPub", "SSDPSRV", "upnphost", "fdPHost")
